...
function sm_autoloader($class,$dir="") {
	include "structure.inc.php";
	$classPathStruct="{/".implode(",/",$classPathStructure).",}";
	foreach($classPath as $_path=>$p){
		...
		$dirs=glob($p.$classPathStruct, GLOB_BRACE | GLOB_ONLYDIR | GLOB_MARK );
		if(is_array($dirs))
		{
...
function sm_app_redirect($location) {
	include "config.inc.php";
	$_baseUrl = sm_Config::get("BASEURL", $baseUrl);
	$base=str_replace("/","\\/",$_baseUrl);
	if(preg_match("/^(http|".$base.")/", $location))
		sm_App::getInstance()->setRedirection($location);
	else 
	{
		sm_App::getInstance()->setRedirection($_baseUrl.$location);
	}
}
	...
	if (isset($_FILES[$key]) && is_uploaded_file($_FILES[$key]['tmp_name']))
	{
		...
		else
		{
			...
			if(!in_array($ext,$allowedExtensions))
			{
				...
				if(!empty($allowedExtensions))
					$msg.=t(" Formats allowed: ").implode(", ", $allowedExtensions);
				$res=false;
			}
			else
			{
...
function sm_deleteDirectory($dir) {
	if (!file_exists($dir)) {
		return true;
	}
	...
	foreach (scandir($dir) as $item) {
		...
		if (!sm_deleteDirectory($dir . DIRECTORY_SEPARATOR . $item)) {
			return false;
		}
