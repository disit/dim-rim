...
/*	function __construct() 
	...
	public function sm_Database($SM_DB_HOST=null, $SM_DB_USER=null, $SM_DB_PASS=null)
	{
		...
		else
		{
			...
			if(isset($dbHost) && isset ($dbUser) && isset($dbPwd))
			{
	...
	*/
	...
	/**
	 * @desc Get the current shared instance of Database 
	 * @return sm_Database
	 */
	...
	/**
	 * @desc Get info about the current database connection
	 * @return <NULL,mixed>
	 */
	function getInfo()
	{
	...
	/**
	 * @desc Get the current active/selected Database
	 * @return String
	 */
	...
	/** 
	 * @desc Get the count of rows in a table, with a condition.
	 * @param $table The table where to compute the number of rows.
	 * @param $where The array of conditions before to compute the number or rows.
	 * @return The number of rows (0 or more).
	 */
	function countOf($table, $where=array())
	{
		$whereCond=1;
		if(!empty($where))
			$whereCond = $this->buildWhereClause($table,$where);
		$result = $this->query("SELECT COUNT(*) as count FROM `$table` WHERE ".$whereCond);
		return $result[0]['count'];
	}
	
	/** 
	 * @desc Get the total count of rows in a table.
	 * @param $table The table where to compute the number of rows.
	 * @return The number of rows (0 or more).
	 */
