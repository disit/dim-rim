...
$sql['user_perms']="CREATE TABLE IF NOT EXISTS `acl_user_perms` (
		...
		`addDate` datetime NOT NULL, 
		PRIMARY KEY  (`ID`), 
		UNIQUE KEY `userID_p` (`userID`,`permID`)
	) ENGINE=MyIsam DEFAULT CHARSET=utf8;";
...
$sql['user_roles']="CREATE TABLE IF NOT EXISTS `acl_user_roles` (
		...
		`addDate` datetime NOT NULL,
		UNIQUE KEY `userID_r` (`userID`,`roleID`)
	) ENGINE=MyIsam DEFAULT CHARSET=utf8;";
...
$sql['AssignAdminRole']="INSERT INTO `acl_user_roles` VALUES (1,00000000000000000001,'".date ("Y-m-d H:i:s")."');";
