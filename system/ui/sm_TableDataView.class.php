...
class sm_TableDataView extends sm_HTML//sm_Table
{
	...
	function __construct($id=null, $opts=array())
	{
		...
		$this->seletectedCmd=isset($opts['seletectedCmd'])?$opts['seletectedCmd']:array();
		$this->headerCmd=isset($opts['headerCmd'])?$opts['headerCmd']:array();
		...
		$this->filters=array();
		$this->title=null;
		$this->ajax=isset($opts['ajax'])?$opts['ajax']:false;
	 ...
	 * @param string $bool
	 */
	function setHeaderCmd($cmd=null)
	{
		$this->headerCmd=$cmd;
	}
	...
	protected function makeCmdBar(){
			$bar=null;
			if($this->seletectedCmd || $this->headerCmd) // && $this->pager && $this->pager->get_total()>0)
			{
				$bar = new sm_HTML();
				...
				foreach($this->headerCmd as $command)
				{
					$name=isset($command['name'])?$command['name']:"";
					$class=isset($command['class'])?$command['class']:"";
				...
				foreach($this->seletectedCmd as $command)
				{
					...
					$toggle = isset($command['data-toggle'])?$command['data-toggle']:"";
					$target = isset($command['data-target'])?$command['data-target']:"";
					$bar->insert("item",'<li><a name="'.$name.'" data-confirm="'.$confirm.'" data-toggle="'.$toggle.'" data-target="'.$target.'" class="tbw_command tbw_command_selection '.$class.'" href="'.$href.'" title="'.$title.'"><i class="'.$icon.'"></i>'.$label.'</a></li>');
				}
	...
	public function TableDataViewFilters_form(sm_Form $form)
	{
		...
		if($this->pager && $this->pager->get_total()>0)
		{
			//$form->addElement(new Element_HTML('<label>'));
			$form->addElement(new Element_Select("", "limit", $this->pager->get_perPageOptions(),array('value'=>$this->pager->get_perPage(),'class'=>'input-sm')));
			$form->addElement(new Element_HTML('<label> entries</label>'));
