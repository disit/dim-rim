...
class sm_Mail implements sm_Module
{
	...
	{
		sm_Config::set("AUTHENTICATED_SMTP_SERVER",array("value"=>"smtp.server.com","description"=>"SMTP server url"));
		...
		sm_Config::set("TEXT_SIGNATURE_EMAIL",array("value"=>"@SITE_NAME","description"=>"Mail signature"));
	...
	static function SendEmail($from, $namefrom, $to, $nameto, $subject, $message, $conf)
	{
		...
		$username = isset($conf['smtpUser'])?$conf['smtpUser']:null; //"test";
		$password = isset($conf['smtpPassword'])?$conf['smtpPassword']:null;//"12345678";
