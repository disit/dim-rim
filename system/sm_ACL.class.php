...
define("ACL_USER_ADMINISTRATOR","00000000000000000001");
define("ACL_USER_AUTHENTICATED","00000000000000000002");
define("ACL_USER_ANONYMOUS","00000000000000000003");
	...
	/*	function ACL($userID = '')
		...
		function getUserRoles($userID=null)
		{
			...
			if($userID==0)
			{
				return array(0=>ACL_USER_ANONYMOUS);
			}
		...
		static public function checkPermission($permKey){
			...
			$roleID=$acl->getRoleIDFromName('System Administrator');
			return $acl->hasPermission($permKey) || $acl->userHasRole($roleID);
		}
		...
		/**
		 * @desc Install a permission in ACL Table 
		 * @param array $perm
		 * ['permID', 'permKey', 'permName', 'description']
		 */
		...
		/**
		 * @desc Install a role in ACL Table
		 * @param array $role
		 * ['roleID', 'roleName', 'description']
		 */
		static public function installRole($role)
		{
			if(!isset($role['roleID']))
				$role['roleID']=null;
			$acl = new sm_ACL();
			if(isset($role['roleName']) && !$acl->getRoleIDFromName($role['roleName']))
			{
				$acl->saveRole($role);
			}
		...
		function saveRole($role){
			if(!isset($role['description']))
				$role['description']="";
			$strSQL = sprintf("REPLACE INTO `acl_roles` SET `ID` = %u, `roleName` = '%s', `description` = '%s'",$role['roleID'],$this->db->escapeValue($role['roleName']),$this->db->escapeValue($role['description']));
			//sm_Logger::write($strSQL);
			$result=$this->db->query($strSQL);
		...
		{
			$this->db->delete("acl_user_roles",array("userID"=>$uid));
			$this->db->delete("acl_user_perms",array("userID"=>$uid));
		}
