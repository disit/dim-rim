...
class sm_MenuConfigController extends sm_ControllerElement
{
	...
	function menu_menus($op=null, $id=null)
	{
		...
		if(!isset($id))
			$id = $menus[0]['id'];
		else if(!$this->model->getMenus($id))
		{
			sm_set_error("Menu id ".$id." does not exist!");
			sm_app_redirect("config/menu/menus");
			return;
		}
		$data['items'] = $this->model->getMenuItems($id);
		$data['menu']=$menus;
	...
	function menu_add($data)
	{
		...
		if(isset($data['name']))
		{
			...
			$menu->setmodule("sm_MenuBar");
			$menu->settpl_var("menu");
			$menu->settype("main_menu");
