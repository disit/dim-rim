        ...
        TableDataView.init=function()
    	{
    		...
    		if(TableDataView.options.ajax)
    		{
    			$table.on('click','#pagination a',function(event){
    				...
    				TableDataView.showLoading();
    				$.get(href,function(html){
    			    	
    			    	$table.find("table tbody").remove();
    			    	$(html).find("tbody").appendTo($table.find("table")); 
    			    	$table.find("table tfoot").remove();
    			    	$(html).find("tfoot").appendTo($table.find("table")); 
    			    	$table.find("table").trigger("update",[true]);
    			    	TableDataView.hideLoading();
    			    });
    				return false;
    			});
    			$table.on('submit',"form#TableDataViewFilters",function(event){
    				...
    				//$table.parent().load(url,data);
    			    //$table.find("tbody")
    				TableDataView.showLoading();
    			    $.post(url,data,function(html){
    			    	$('form#TableDataViewFilters input[type=submit]').removeAttr("disabled");
    			    	$table.find("table tbody").remove();
    			    	$(html).find("tbody").appendTo($table.find("table")); 
    			    	$table.find("table tfoot").remove();
    			    	$(html).find("tfoot").appendTo($table.find("table"));
    		...
    		$table.find('#header-cmd a.tbw_command').on('click',function(event)
				{
						...
						event.stopPropagation();
						if($(this).hasClass("tbw_command_selection"))
						{	
	    	...
	    	TableDataView.showLoading=function() {
	        ...
	        TableDataView.hideLoading=function() {
