	...
	public function render() {
		...
		for($e = 0; $e < $elementSize; ++$e) {
			...
			elseif($element instanceof Element_Button) {
				...
				if($element instanceof Element_CKEditor)
					echo '<div class="form-group'.$span_class.'">', $this->renderLabel($element), $element->render(),$this->renderDescriptions($element), '</div>';
				else 
					echo '<div class="form-group'.$span_class.'">', $this->renderLabel($element), "<div id='".$element->getAttribute("id")."'>",$element->render(),"</div>",$this->renderDescriptions($element), '</div>';
				++$elementCount;
