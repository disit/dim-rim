...
sm_Logger::removeLog();
	...
	if(isset($_GET['q']))
	{
		...
		else if($_GET['q']=="app" && isset($_GET['class']))//update a class of an application
		{
			echo "Updating ".$_GET['class']."<br>";
			$install_app->setModule($_GET['class']);
			$install_app->setMode('update_app');
		}
		else if($_GET['q']=="app" && isset($_GET['name'])) //install all application by name
		{
			echo "Installing App ".$_GET['name']."<br>";	
			$install_app->setMode('install_app');
			$install_app->setModule($_GET['name']);
		}
		else
