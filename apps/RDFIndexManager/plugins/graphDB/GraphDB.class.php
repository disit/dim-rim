...
define("GRAPHDBHOME","/home/ubuntu/apache-tomcat-7.0.61");
define("GRAPHDBHOMEBIN","/home/ubuntu/apache-tomcat-7.0.61/bin");
define("GRAPHDBSTART","/home/ubuntu/apache-tomcat-7.0.61/bin/startup.sh");
define("GRAPHDBSTOP","/home/ubuntu/apache-tomcat-7.0.61/bin/shutdown.sh");

define("GRAPHDBREPOSITORY","/root/.aduna/openrdf-sesame/repositories/");
class GraphDB implements sm_Module
{
	...
	function create($name=null){
		...
		$tpl->newTemplate("memory_GraphDB", $filename);
		$tpl->addTemplateData("memory_GraphDB", array("name"=>$name));
		$body = $str = $tpl->display("memory_GraphDB");
		$urlName = "http://www.disit.org%23".str_replace(" ", "_", $name);
		$url = sm_Config::get('GraphDBOPENRDFURL',"http://localhost:8080/openrdf-sesame/");
	...
	function deleteRepository($name=null)
	{
		if(!$name || !$this->exist($name))
			return false;
		sm_Logger::write("GraphDB delete: ".$name);
	}
	...
	function flush($name)
	{
		...
		$triple = '<http://www.example.com> <http://www.ontotext.com/GraphDB/system#flush> "" .';
	...
	function creategeospatial($name)
	{
		...
		//$data['update']='INSERT DATA { _:b1 <http://www.ontotext.com/owlim/geo#createIndex> _:b2. }';
		//$data=' _:b1 <http://www.ontotext.com/owlim/geo#createIndex> _:b2. ';
		$data['query']=$query;
		//$data="{'update':'INSERT DATA { _:b1 <http://www.ontotext.com/owlim/geo#createIndex> _:b2. }";
	...
	{
		sm_Config::set('GraphDBOPENRDFWORKBENCH',array("value"=>"http://localhost:8080/openrdf-workbench/","description"=>"OpenRDF Workbench Address"));
		SiiMobilityRepositoryManager::addType("GraphDB");
		sm_Config::set('GraphDBOPENRDFURL',array("value"=>"http://localhost:8080/openrdf-sesame/","description"=>"OpenRDF Sesame Database Address"));
	...
	{
		sm_Config::delete('GraphDBOPENRDFWORKBENCH');
		sm_Config::delete('GraphDBOPENRDFURL');
		SiiMobilityRepositoryManager::removeType("GraphDB");
		return true;
	}
	...
	private function _getScript(SiiMobilityRepository $generation){
		...
		$restcallUrlCreate = "http://localhost".sm_Config::get("BASEURL","")."api/GraphDB/create/";
		$restcallUrlFlush = "http://localhost".sm_Config::get("BASEURL","")."api/GraphDB/flush/";
		$restcallUrlCommit="http://localhost".sm_Config::get("BASEURL","")."api/IndexGenerator/commit/";
		$restcallUrlGeoIndex = "http://localhost".sm_Config::get("BASEURL","")."api/GraphDB/create/geospatial/";
		...
		$sesameUrl = sm_Config::get('GraphDBOPENRDFURL',"http://localhost:8080/openrdf-sesame/");
		$scriptPath= '/opt/indexgenerator/script/owlim.sh';//"/opt/GraphDB/getting-started/example.sh";
		...
		//$fileName = '/home/ubuntu/Desktop/GraphDB-lite-5.4.6287/getting-started/generateindex.sh';
		...
		$fileName = $fileDir.'/GraphDB_'.$generation->getID().'.sh';
		...
		fwrite($fp, "	clear\n");
				...
				fwrite($fp, "echo \"*********** START GraphDB SCRIPT *****************\" >> \$progressFile\n");
				...
				fwrite($fp, "\n");
				fwrite($fp, "echo \"\" >> \$progressFile\n");
				fwrite($fp, "echo \"*************** START GraphDB COMMIT *************\" >> \$progressFile\n");
				fwrite($fp, "echo \"\" >> \$progressFile\n");
				if(!$this->exist($repositoryID))
				{
					...
					if(!empty($parentID))
					{
						...
						if($N>0)
						{
							fwrite($fp, "echo \"Sleep for 10 secs\" >> \$progressFile\n");
							fwrite($fp, "sleep 10\n");
						}
					}
				}
	...
	private function _getData($dataType, $idGeneration) {
		...
		/*	$line[]="# Assign mode & permissions to ".$target."\n";
			...
			$line[]="echo \"Restarting Graph DB\"\n";
			$line[]="# Restarting Graph DB Server \n";
			$line[]=GRAPHDBSTART."\n";
			foreach ($line as $l){
				fwrite($fp,$l);
			}
		...
		private function _restartScript($fp,Generation $generation)
		{
			...
			$source = $repositoryFolder.$generation->getRepositoryID()."/storage";
			if(!$fp)
				return;
			$line[]= "echo \"Stopping Graph DB\"\n";
			$line[]="# Stopping Graph DB Server\n";
			$line[]=GRAPHDBSTOP."\n";
			$line[]= "if [ -f ".$source."/lock ] ; then\n";
			...
			/*	$line[]="# Assign mode & permissions to ".$target."\n";
			 $line[]="chown -R tomcat7:tomcat7 ".$target."\n";
			 $line[]="chmod 755 -R ".$target."\n";*/
			$line[] = "echo \"Sleep for 10 secs\" >> \$progressFile\n";
			$line[] = "sleep 10\n";
			$line[]="echo \"Restarting Graph DB\"\n";
			$line[]="# Restarting Graph DB Server \n";
			$line[]=GRAPHDBSTART."\n";
