...
class GraphDBView extends sm_ViewElement
{
	...
	public function build() {
		...
		if(isset($this->model))
		{
			...
			$html->insert("tool_frame","<iframe src='".$url."' id='GraphDB_tool_view' ></iframe>");
			$html->addCss("GraphDB.css","main",GraphDBPlugin::instance()->getFolderUrl("css"));
			//$this->addView();
		}
	}
	...
	static function menu(sm_MenuManager $menu)
	{
		...
		$menu->setSubLink("RDF Tools","GraphDB Workbench","GraphDB/openrdf-workbench");
	}
	...
	public function onExtendView(sm_Event &$event)
	{
		...
		if(is_object($obj))
		{
			if(get_class($obj)=="SiiMobilitySettingsView")
			{
				...
				$panel->insert(sm_Form::buildForm("GraphDB_config", $this));
				$panel->addCss("GraphDB.css","main",GraphDBPlugin::instance()->getFolderUrl("css"));
			}
			if(get_class($obj)=="SiiMobilityIndexGeneratorView" && ($obj->getOp()=="editor" || $obj->getOp()=="welcome"))
			{
				...
				{
					$m=array(
							'link'=>'GraphDB/openrdf-workbench',
							'description'=>"GraphDB",
							'type'=>"GraphDB",
							'title'=>"GraphDB RDF Sesame",
							'class'=>"col-md-2"
					);
					$icon = new sm_HTML();
		...
		{
			$ui=$obj->getUIView();
			$ui->addCSS("GraphDB.css",$ui->getTemplateId(),GraphDBPlugin::instance()->getFolderUrl("css"));
		}
	}
	
	
	function GraphDB_config_form($form){
		$form->configure(array(
				"prevent" => array("bootstrap", "jQuery", "focus"),
				//	"view" => new View_Vertical,
				//"labelToPlaceholder" => 0,
				"action"=>"GraphDB/settings"
		));
