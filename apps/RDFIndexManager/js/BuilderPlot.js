...
var plot={};
var plotdata={};
var plotRefreshUrl=null;
var n=20; //Max number of data to plot
var refreshTime = 10000; //10 secs;
...
function renderBuilderGraph(id,title,options) {
	    if (plot[id]) {
	        plot[id].destroy();
	    }
	    ...
	    options.title={
				   text:title,
				   fontSize:"12px"// title for the plot,
			   };
	    plot[id] = $.jqplot(id, [storedData], options);
	    
	}

function doBuilderGraphUpdate() {
	if(!plotRefreshUrl)
		return;
    ...
    if (data) {
    	...
    	for (p in data) {
            ...
            renderBuilderGraph(p,plot[p].title.text,options);
    	}  
	...
	if($('#BuilderView').is(":visible"))
	{
		...
		builderTimeout = setTimeout(doBuilderGraphUpdate, refreshTime);
	}
	else
		clearTimeout(builderTimeout);
}
