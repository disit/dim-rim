...
class Versioner {
	...
	static function getResourceVersionsArray($directory, $getLast=false) {
		...
		if(!file_exists(self::$baseVolume.$directory)) {
			return $out;
		}
		
		// Get the directory listing
		$dirList = scandir(self::$baseVolume.$directory, 1);
	...
	static function getResourcesByTimeInterval($directory, $from, $to)
	{
		// Set timezone
		date_default_timezone_set('Europe/Rome');
				// Start date
		$startDate = $date = $from; //'2015-04-01 15:00:00';
		// End date
		$end_date = $to; //'2015-05-01 13:00:00';
		$path=$directory;//'/media/Triples/AVM/avm_linea17/';
	...
	//	echo '<div>CWD is '.getcwd().'</div>';
		while (strtotime($date) < strtotime($end_date)) {
			...
			$date_ = date ("Y_m/d", strtotime($date));
			//$list = glob($path.$date_."/*/*/*");
			$list = glob($path.$date_."/*/*",GLOB_ONLYDIR);
			
			foreach($list as $l=>$f)
			{
				$mtime = filemtime($f);
				if($mtime>=strtotime($startDate) && $mtime<=strtotime($end_date))
				{
					$folders[]=str_replace($path, "",$f);
				}
			}
