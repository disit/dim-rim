		...
		if ( is_file($file) || is_link($file)) {
			if ( is_readable($file) || is_link($file)) {
				...
				if(!is_link($file))
					...
					$archive=readlink($file);
				
				$totalLines=count($archive);
				//$lines=10;
				if(isset($_SESSION[$file]['lines']))
					$lines = $totalLines-$_SESSION[$file]['lines'];
				else
					$lines= $totalLines;
				$_SESSION[$file]['lines'] = $totalLines;
				
				$cont="";
				for($g=0;$g<$totalLines;$g++){
